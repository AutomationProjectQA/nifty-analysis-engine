...
| P5-5 | Holiday calendar + feed-freshness gate; move LLM/Telegram/order off the cycle thread (non-blocking `collect()`) | `MarketScheduler`, `MarketCollectorService` | No runs on holidays; stale snapshot rejected; cycle never overruns/drops a minute | M | ◐ holiday-skip + freshness gate DONE; non-blocking `collect()` DEFERRED (concurrency risk) |
