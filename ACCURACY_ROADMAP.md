...
| P5-2 | **Multi-instrument:** `instrument` dimension (symbol, strike step, lot size, expiry rule); run pipeline per instrument — **Bank Nifty first** | clients, DTOs, entities, `DecisionAgent` | Bank Nifty signals generated + backtested; Nifty path unchanged | L | ◐ Slice1 InstrumentRegistry (NIFTY/BANKNIFTY specs) + Slice2 V1.7 migration + entity instrument field DONE; Slice 3 (instrument-aware clients; SimulatedDataClient real Bank Nifty; AngelOne Nifty-unchanged + non-Nifty safe fallback) DONE. Slice 4 (per-instrument collector loop + instrument-scoped reads across 6 services/4 repos + test updates) REMAINS |
