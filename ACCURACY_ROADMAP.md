...
| ID | Task | Files | Acceptance criteria | Effort | Status |
|----|------|-------|---------------------|--------|--------|
| P5-1 | **Regime-aware DEFINED-RISK strategies** (credit/debit spreads + iron condor, routed by regime) | StrategyType/Selector/Payoff/Builder, TradeLeg+migration, DecisionAgent, MarketCollectorService | Multi-leg signals emit + resolve on NET P&L with capped risk; 133 tests; gated `nifty.strategy.spreads-enabled` (default off); single-leg unchanged | L | ☑ (paper-tracked; live multi-leg ORDER placement not wired) |
| P5-2 | **Multi-instrument:** instrument dimension + per-instrument pipeline (Bank Nifty) | clients, DTOs, entities, repos, DecisionAgent | Bank Nifty signals on simulated; Nifty path unchanged; 123 tests green | L | ☑ (live Angel BN fetch falls back to simulated until VM-validated; BANKNIFTY config-gated) |
